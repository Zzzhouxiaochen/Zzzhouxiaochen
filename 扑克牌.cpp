...
{
	int q = 0;s
	if (((max(T[10][10]) - 1) == T[0][0] || (max(T[10][10]) - 1) == T[1][0] || (max(T[10][10]) - 1) == T[2][0])
		&& ((max(T[10][10]) - 2) == T[0][0] || (max(T[10][10]) - 2) == T[1][0] || (max(T[10][10]) - 2) == T[2][0]))
		return 1;
	else
		return 0;
}
...
{ 
	...
	printf("同花顺概率为%d,顺子顺概率为%d,同点顺概率为%d,对子顺概率为%d,杂牌顺概率为%d,", e1 / times, e2 / times, e3 / times, e4 / times, e5 / times);
	system("pause");
}